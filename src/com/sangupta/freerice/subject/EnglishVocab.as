...
package com.sangupta.freerice.subject {
	...
	public class EnglishVocab extends BaseSubject {
		
		override public function getAnswer(question:String):String {
			question = StringUtil.trim(question);
			var statement:SQLStatement = DBManager.getStatement('SELECT * FROM vocab WHERE word=:word');
			statement.parameters[":word"] = question;
			statement.execute();
			var result:SQLResult = statement.getResult();
			if(result != null && result.data != null) {
				return result.data[0].meaning;
			}
		...
		override public function saveAnswer(question:String, answer:String):void {
			try {
				var statement:SQLStatement = DBManager.getStatement('INSERT INTO vocab (word, meaning) VALUES (:word, :meaning)');
				statement.parameters[":word"] = question;
				statement.parameters[":meaning"] = answer;
				statement.execute();
			} catch(e:Error) {
